...
class AdminReportController extends Controller
{
    ...
    public function removeReportAction($args) {
        ...
        if(!$login->isLoggedIn()) {
            $response = $this->createResponseArray(true, 'error', 'You must be logged in to remove reports.');
            $this->doResponse($response, $format);
        }

        try {
            $reportExists = $this->container->storage->reportExists($id);
        } catch(StorageException $e) {
            $this->doDbErrorResponse($format);
        }
        if(!$reportExists) {
            $response = $this->createResponseArray(true, 'error', 'That report does not exist.');
            $this->doResponse($response, $format);
        }

        $count = 0;
        try {
            $this->container->storage->removeReport($id);
        } catch(StorageException $e) {
            $this->doDbErrorResponse($format);
        }
        $response = $this->createResponseArray(false, 'success', 'Report has been successfully deleted.');
        $this->doResponse($response, $format);
    }

    public function createReportAction($args) {
